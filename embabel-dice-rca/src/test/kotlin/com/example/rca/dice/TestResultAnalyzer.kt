...
class TestResultAnalyzer {
        ...
        fun analyze(reports: List<TestExecutionReport>): TestAnalysisResult {
                ...
                if (failedWithCov.isNotEmpty()) {
                        ...
                        val thresholds =
                                failedWithCov
                                        .mapNotNull { (r, _) ->
                                                r.verification?.requiredKeywordCoverage
                                        }
                                        .distinct()
                ...
                if (allMissing.isNotEmpty()) {
                        ...
                        val top =
                                counts.entries.sortedByDescending { it.value }.take(5).map {
                                        it.key
                                }
                        findings.add(
                                AnalysisFinding(
                                        ...
                                        message =
                                                "Certain keywords are often missing in failed verifications.",
                                        detail =
                        ...
                        suggestions.add(
                                ParamSuggestion(
                                        ...
                                        current = null,
                                        suggested =
                                                "Add or allow synonyms: ${top.take(3).joinToString(", ")}",
                                        reason =
                ...
                val withTemp =
                        reports.mapNotNull { r ->
                                val t =
                                        (r.metadata["ai_params"] as? Map<*, *>)?.get("temperature")
                                                ?.let { (it as? Number)?.toDouble() }
                                                ?: (r.metadata["ai_temperature"] as? Number)
                                                        ?.toDouble()
                                ...
                                if (avgFailTemp > avgPassTemp + 0.1) {
                                        findings.add(
                                                AnalysisFinding(
                                                        ...
                                                        message =
                                                                "Failures correlate with higher AI temperature.",
                                                        detail =
                ...
                if (errors.isNotEmpty()) {
                        ...
                        findings.add(
                                AnalysisFinding(
                                        ...
                                        message =
                                                "Some tests ended in ERROR (not verification failure).",
        ...
        private fun buildSummary(
                ...
                return buildString {
                        ...
                        appendLine("==============")
                        appendLine(
                                "Tests: $total total, $passed passed, $failed failed, $err errors."
                        )
                        ...
                        if (suggestions.isNotEmpty()) {
                                ...
                                suggestions.forEach { s ->
                                        appendLine(
                                                "  - ${s.param}: ${s.suggested} (${s.reason.take(80)}...)"
                                        )
                                }
                        }
                }
        ...
        fun formatAsMarkdown(
                ...
                reports: List<TestExecutionReport> = emptyList()
        ): String {
                return buildString {
                        ...
                        if (result.findings.isNotEmpty()) {
                                ...
                                result.findings.forEach { f ->
                                        ...
                                        if (f.scenarioId != null)
                                                appendLine("  - Scenario: ${f.scenarioId}")
                                        appendLine()
                                }
                        }
                        if (result.suggestions.isNotEmpty()) {
                                ...
                                appendLine()
                                result.suggestions.forEach { s ->
                                        appendLine("- **${s.param}**: ${s.reason}")
                                }
                                appendLine()
                        }
                        if (reports.isNotEmpty()) {
                                appendLine("## Per-test: actual LLM output vs expected")
                                appendLine()
                                reports.forEach { r ->
                                        val scenario = r.metadata["scenario_id"]?.toString() ?: "—"
                                        val statusIcon =
                                                when (r.status) {
                                                        TestStatus.PASSED -> "✅"
                                                        TestStatus.FAILED -> "❌"
                                        ...
                                        appendLine("### $statusIcon ${r.testName}")
                                        appendLine()
                                        appendLine(
                                                "- **Scenario:** $scenario | **Status:** ${r.status}"
                                        )
                                        appendLine()
                                        // Expected
                                        ...
                                        appendLine("**Expected:**")
                                        appendLine(
                                                "- Keywords: ${v?.expectedKeywords?.joinToString(", ") ?: "—"}"
                                        )
                                        appendLine("- Component: ${v?.expectedComponent ?: "—"}")
                                        appendLine("- Cause type: ${v?.expectedCauseType ?: "—"}")
                                        appendLine(
                                                "- Required keyword coverage: ${v?.let { "${(it.requiredKeywordCoverage * 100).toInt()}%" } ?: "—"}"
                                        )
                                        appendLine()
                                        // Actual LLM output (root cause analysis)
                                        val actual =
                                                r.analysis?.rootCauseAnalysis
                                                        ?: v?.actualRootCause ?: "(none)"
                                        appendLine("**Actual LLM output (root cause analysis):**")
                                        appendLine("```")
                                        appendLine(actual)
                                        appendLine("```")
                                        appendLine()
                                        // Verification summary for this test
                                        if (v != null) {
                                                appendLine(
                                                        "**Verification:** Coverage ${v.keywordCoveragePercent}% | " +
                                                                "Found: ${v.keywordsFound.joinToString(", ").ifBlank { "—" }} | " +
                                                                "Missing: ${v.keywordsMissing.joinToString(", ").ifBlank { "—" }} | " +
                                                                "Component: ${if (v.componentIdentified) "✓" else "✗"} | " +
                                                                "Cause type: ${if (v.causeTypeIdentified) "✓" else "✗"}"
                                                )
                                                appendLine()
                                        }
                                }
