...
class AllScenariosTest {
    ...
    fun `scenario - database pool exhaustion`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    rootCause = "Connection pool exhausted due to slow queries or connection leaks",
                    resolution = "Check slow queries, increase pool size, fix connection leaks",
                    affectedServices = listOf("api-service")
                )
            ),
            dependencies = listOf(
                ServiceDependency("api-service", "postgres", "database", true, "HikariCP max=100")
            )
        ))
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            name = "API P95 Latency Alert",
            service = "api-service",
            message = "P95 latency exceeded 500ms. Current: 2450ms",
            metricsToQuery = listOf("jvm.db.pool.active{service:api-service}"),
        ...
        val verification = testFramework.verifyConclusion(result, ExpectedRootCause(
            keywords = listOf("connection pool", "exhausted", "database", "timeout"),
            component = "database"
        ))
    ...
    fun `scenario - database deadlock`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    rootCause = "Concurrent updates accessing rows in different order",
                    resolution = "Fix transaction ordering, use SELECT FOR UPDATE",
                    affectedServices = listOf("inventory-service")
                )
            )
        ))
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            message = "Error rate exceeded threshold with transaction failures",
            logQueries = listOf("service:inventory-service deadlock")
        ))

        val verification = testFramework.verifyConclusion(result, ExpectedRootCause(
            keywords = listOf("deadlock", "transaction", "concurrent", "lock"),
            component = "database"
        ))
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "Heavy I/O operations saturating disk",
                    symptoms = listOf(
                        "system.io.util > 90%",
                        "High I/O await times",
                        "Slow database queries"
                    ),
                    rootCause = "Backup or heavy batch job consuming I/O bandwidth",
                    resolution = "Reschedule backup to off-peak, use I/O throttling",
                    affectedServices = listOf("order-service", "postgres")
                )
            )
        ))
    ...
    fun `scenario - kubernetes oom killed`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "Pod terminated due to memory limit exceeded",
                    symptoms = listOf(
                    ...
                    rootCause = "Memory leak or insufficient memory limits",
                    resolution = "Fix memory leak, increase limits, add GC tuning",
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            metricsToQuery = listOf("kubernetes.memory.usage{deployment:order-service}")
        ))
    ...
    fun `scenario - kubernetes node pressure`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "Node under memory pressure, evicting pods",
                    symptoms = listOf(
                        "Pod evictions",
                    ...
                    rootCause = "Node memory exhausted, triggering evictions",
                    resolution = "Add nodes, reduce pod memory, enable autoscaler",
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            message = "Multiple services experiencing errors simultaneously",
    ...
    fun `scenario - cpu throttling`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "Container CPU limits causing throttling",
                    symptoms = listOf(
                        "Latency spikes during peak traffic",
                        "kubernetes.cpu.cfs.throttled.seconds high",
                        "CPU at limit but node has capacity"
                    ),
                    rootCause = "CPU limits too restrictive for workload",
                    resolution = "Increase CPU limits, enable VPA",
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            metricsToQuery = listOf("kubernetes.cpu.cfs.throttled.seconds{deployment:search-service}")
        ))
    ...
    fun `scenario - downstream service failure`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "Downstream dependency failure causing cascade",
                    symptoms = listOf(
                        "Timeout errors to downstream service",
                        "Circuit breaker OPEN",
                        "503 errors from gateway"
                    ),
                    rootCause = "Downstream service failure cascading upstream",
                    resolution = "Fix downstream service, circuit breaker limits blast radius",
                    affectedServices = listOf("checkout-service", "payment-service")
                )
            ),
            dependencies = listOf(
                ServiceDependency("checkout-service", "payment-service", "http", true)
            )
        ))
        ...
        val verification = testFramework.verifyConclusion(result, ExpectedRootCause(
            keywords = listOf("payment-service", "downstream", "circuit breaker"),
    ...
    fun `scenario - dns resolution failure`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "CoreDNS or NodeLocal DNS issues",
                    symptoms = listOf(
                        "UnknownHostException errors",
                        "DNS lookup timeouts",
                        "Service discovery failures"
                    ),
                    rootCause = "DNS infrastructure overwhelmed or crashed",
                    resolution = "Scale CoreDNS, check NodeLocal DNS pods",
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            logQueries = listOf("UnknownHostException OR 'Name does not resolve'")
        ))
    ...
    fun `scenario - network partition`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "Network split between availability zones",
                    symptoms = listOf(
                    ...
                    resolution = "Check network connectivity, failover to healthy AZ",
                    affectedServices = listOf("cross-AZ services")
                )
            )
        ))
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            logQueries = listOf("'connection refused' OR 'no route to host'")
        ))

        val verification = testFramework.verifyConclusion(result, ExpectedRootCause(
            keywords = listOf("network", "partition", "AZ", "connectivity"),
            component = "network"
        ))
    ...
    fun `scenario - envoy sidecar failure`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    rootCause = "Envoy proxy OOM or configuration issue",
                    resolution = "Increase sidecar memory, check Istio config",
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            logQueries = listOf("istio-proxy OOM OR 'upstream connect error'")
        ))
    ...
    fun `scenario - redis cache failure`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "Cache unavailable, fallback to database",
                    symptoms = listOf(
                        "Cache miss rate 100%",
                        "Redis connection errors",
                        "Database load spike"
                    ),
                    rootCause = "Redis server down or unreachable",
                    resolution = "Restart Redis, check Sentinel/Cluster, add fallback",
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            metricsToQuery = listOf("app.cache.miss_rate{service:product-service}")
        ))
    ...
    fun `scenario - kafka consumer lag`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "Consumer falling behind, processing delays",
                    symptoms = listOf(
                        "Consumer lag increasing",
                        "Data freshness SLA breach",
                        "Slow message processing"
                    ),
                    rootCause = "Consumer processing too slow (often N+1 queries)",
                    resolution = "Optimize processing, batch queries, scale consumers",
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            metricsToQuery = listOf("kafka.consumer.lag{consumer_group:event-processor}")
        ))
    ...
    fun `scenario - gc storm`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "Excessive GC causing application pauses",
                    symptoms = listOf(
                        "Long GC pauses (seconds)",
                        "GC overhead > 80%",
                        "High latency spikes"
                    ),
                    rootCause = "Memory pressure causing constant GC",
                    resolution = "Fix memory leak, increase heap, tune GC",
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            metricsToQuery = listOf("jvm.gc.pause_time{service:report-service}")
        ))

        val verification = testFramework.verifyConclusion(result, ExpectedRootCause(
            keywords = listOf("GC", "garbage collection", "heap", "memory"),
            component = "jvm"
        ))
    ...
    fun `scenario - thread pool exhaustion`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "Thread pool saturated by blocking calls",
                    symptoms = listOf(
                        "Thread pool at max",
                        "Request rejections",
                        "Threads blocked on I/O"
                    ),
                    rootCause = "Slow downstream causing thread blocking",
                    resolution = "Add timeouts, use async I/O, implement bulkhead",
                    affectedServices = listOf("api-service")
                )
            )
        ))
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            metricsToQuery = listOf("jvm.thread.pool.active{service:api-service}")
        ))
    ...
    fun `scenario - ssl certificate expiry`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "Expired certificate causing TLS failures",
                    symptoms = listOf(
                    ...
                    resolution = "Renew certificate immediately",
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            logQueries = listOf("SSLHandshakeException OR 'certificate expired'")
        ))
    ...
    fun `scenario - secret rotation failure`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "Credential rotation not propagated to pods",
                    symptoms = listOf(
                        "Authentication failures to database",
                        "Password authentication failed",
                        "Pods using stale credentials"
                    ),
                    rootCause = "Secret rotated but pods not restarted",
                    resolution = "Restart pods, implement secret sync",
        ...
        val verification = testFramework.verifyConclusion(result, ExpectedRootCause(
            keywords = listOf("secret", "password", "credential", "rotation"),
            component = "security"
        ))
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "ConfigMap change not propagated to all pods",
                    symptoms = listOf(
                        "Intermittent failures",
                        "Some pods succeed, some fail",
                        "Different config versions across pods"
                    ),
                    rootCause = "Config change not propagated, pods not restarted",
                    resolution = "Rolling restart, implement config reloader",
                    affectedServices = listOf("order-service")
                )
            )
        ))
    ...
    fun `scenario - third party api degradation`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "External API experiencing issues",
                    symptoms = listOf(
                        "Timeouts to external API",
                        "Elevated error rates from provider",
                        "Circuit breaker opens"
                    ),
                    rootCause = "Third-party service degradation (outside our control)",
                    resolution = "Enable fallbacks, monitor provider status",
                    affectedServices = listOf("checkout-service")
                )
            )
        ))
        ...
        val verification = testFramework.verifyConclusion(result, ExpectedRootCause(
            keywords = listOf("Stripe", "third-party", "external", "provider"),
            component = "external"
        ))
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    id = "fp-ratelimit",
                    name = "Rate Limiting",
                    description = "Rate limits hit due to traffic spike or abuse",
                    symptoms = listOf(
                        "429 Too Many Requests",
                        "Rate limit bucket exhausted",
                        "Legitimate traffic affected"
                    ),
                    rootCause = "Shared rate limit consumed by abusive client",
                    resolution = "Implement per-client limits, block abusers",
    ...
    fun `scenario - elasticsearch cluster issue`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "ES cluster degraded due to node failure",
                    symptoms = listOf(
                        "Cluster status RED",
                        "Unassigned shards",
                        "Search timeouts/partial results"
                    ),
                    rootCause = "Data node failure causing shard unavailability",
                    resolution = "Add/restart nodes, rebalance shards",
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            metricsToQuery = listOf("elasticsearch.cluster.health.status{cluster:search-cluster}")
        ))

        val verification = testFramework.verifyConclusion(result, ExpectedRootCause(
            keywords = listOf("Elasticsearch", "cluster", "shard", "node"),
            component = "search"
        ))
    ...
    fun `scenario - investigation workflow latency`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            architecture = SystemArchitecture(
                ...
                services = listOf(
                    ServiceInfo("api-service", "API Gateway", "gateway", listOf("inventory-service"), true),
                    ServiceInfo("inventory-service", "Inventory", "service", listOf("postgres"), true)
                )
            ),
            dependencies = listOf(
                ServiceDependency("api-service", "inventory-service", "http", true),
                ServiceDependency("inventory-service", "postgres", "database", true)
            ),
            failurePatterns = listOf(
                FailurePattern(
                    id = "fp-cascade",
                    name = "Database Blocking Queries",
                    description = "Autovacuum or long queries blocking normal operations",
                    symptoms = listOf("High query latency", "Lock wait timeouts"),
                    rootCause = "Database maintenance or blocking operation",
                    resolution = "Check autovacuum, long-running queries",
                    affectedServices = listOf("inventory-service", "api-service")
                )
            )
        ))
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            logQueries = listOf("service:api-service", "service:inventory-service", "autovacuum")
        ))

        val verification = testFramework.verifyConclusion(result, ExpectedRootCause(
            keywords = listOf("autovacuum", "postgres", "database", "blocking"),
            component = "database"
        ))
    ...
    fun `scenario - investigation workflow errors`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            dependencies = listOf(
                ServiceDependency("order-service", "auth-service", "http", true)
            ),
            failurePatterns = listOf(
                FailurePattern(
                    id = "fp-auth-config",
                    name = "Auth Configuration Issue",
                    description = "OAuth config change invalidates tokens",
                    symptoms = listOf("401 errors spike", "issuer_mismatch", "auth-service shows no errors"),
                    rootCause = "Config change removed valid OAuth issuer",
                    resolution = "Revert config, gradual migration",
                    affectedServices = listOf("all authenticated services")
                )
            ),
            pastIncidents = listOf(
                PastIncident(
                    id = "INC-001",
                    ...
                    durationMinutes = 20,
                    severity = "HIGH",
                    symptoms = listOf("401 everywhere", "auth-service healthy"),
                    rootCause = "Removed old issuer from allowed list",
                    resolution = "Added back old issuer",
                    lessonsLearned = listOf("When auth-service healthy but 401s spike, check config")
                )
            )
        ))
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            logQueries = listOf("service:order-service 401", "service:auth-service issuer")
        ))
    ...
    fun `scenario - investigation workflow intermittent`() {
        ...
        testFramework.loadPriorKnowledge(contextId, PriorKnowledge(
            failurePatterns = listOf(
                FailurePattern(
                    ...
                    description = "NodeLocal DNSCache failure on specific node",
                    symptoms = listOf(
                        "Intermittent failures (not all requests)",
                        "Errors correlate to specific pods",
                        "DNS resolution errors"
                    ),
                    rootCause = "NodeLocal DNSCache crashed on one node",
                    resolution = "Restart DNS cache, check node resources",
                    affectedServices = listOf("pods on affected node")
                )
            )
        ))

        mockDatadog.loadScenario("inv-intermittent", createInvestigationIntermittentScenario())
        ...
        val result = testFramework.simulateAlert(contextId, TestAlert(
            ...
            logQueries = listOf("'Name does not resolve' OR UnknownHostException")
        ))
    ...
    private fun createDbPoolScenario() = scenario("db-pool") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "api-service", "api-1", "error",
                "TimeoutError: Connection pool exhausted after 5000ms",
                mapOf("error.type" to "TimeoutError"))
        ))
    }

    private fun createDeadlockScenario() = scenario("deadlock") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "inventory-service", "inv-1", "error",
                "PSQLException: ERROR: deadlock detected",
                mapOf("error.type" to "PSQLException"))
        ))
    }

    private fun createDiskIoScenario() = scenario("disk-io") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "postgresql", "db-1", "warn",
                "I/O wait high: autovacuum and backup running concurrently",
                mapOf("io.util" to 99))
        ))
    }

    private fun createK8sOomScenario() = scenario("k8s-oom") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "kubernetes", "node-1", "error",
                "Container order-service OOMKilled",
                mapOf("kubernetes.event.reason" to "OOMKilled"))
        ))
    }

    private fun createNodePressureScenario() = scenario("node-pressure") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "kubernetes", "node-3", "warn",
                "Evicting pod due to node memory pressure",
                mapOf("kubernetes.event.reason" to "Evicted"))
        ))
    }

    private fun createCpuThrottleScenario() = scenario("cpu-throttle") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "search-service", "search-1", "warn",
                "CPU throttling detected: 25 seconds throttled in last minute",
                mapOf("cpu.throttled_seconds" to 25))
        ))
    }

    private fun createDownstreamScenario() = scenario("downstream") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "checkout-service", "checkout-1", "error",
                "Failed to connect to payment-service: Connection timeout",
                mapOf("downstream.service" to "payment-service")),
            LogEntry("2", now, "checkout-service", "checkout-1", "warn",
                "CircuitBreaker 'payment-service' state changed: CLOSED -> OPEN",
                mapOf("circuit.name" to "payment-service"))
        ))
    }

    private fun createDnsScenario() = scenario("dns") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "api-gateway", "gw-1", "error",
                "UnknownHostException: order-service.production.svc.cluster.local",
                mapOf("error.type" to "UnknownHostException"))
        ))
    }

    private fun createNetworkPartitionScenario() = scenario("partition") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "network-monitor", "mon-1", "error",
                "Network partition detected between az-1 and az-2",
                mapOf("partition.source" to "az-1", "partition.dest" to "az-2"))
        ))
    }

    private fun createEnvoyScenario() = scenario("envoy") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "istio-proxy", "payment-1", "error",
                "Envoy proxy terminated: OOMKilled",
                mapOf("container" to "istio-proxy", "termination.reason" to "OOMKilled"))
        ))
    }

    private fun createRedisScenario() = scenario("redis") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "product-service", "prod-1", "error",
                "RedisConnectionException: Unable to connect to redis-primary:6379",
                mapOf("error.type" to "RedisConnectionException"))
        ))
    }

    private fun createKafkaLagScenario() = scenario("kafka") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "event-processor", "proc-1", "warn",
                "Consumer lag critical: 500,000 messages behind",
                mapOf("kafka.lag" to 500000))
        ))
    }

    private fun createGcScenario() = scenario("gc") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "report-service", "report-1", "error",
                "GC overhead limit exceeded - spent 85% of time in GC",
                mapOf("gc.overhead_percent" to 85))
        ))
    }

    private fun createThreadPoolScenario() = scenario("threads") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "api-service", "api-1", "error",
    ...
    private fun createSslScenario() = scenario("ssl") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "api-gateway", "gw-1", "error",
    ...
    private fun createSecretRotationScenario() = scenario("secret") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "user-service", "user-1", "error",
                "PSQLException: FATAL: password authentication failed for user 'user_service_app'",
                mapOf("error.type" to "PSQLException"))
        ))
    }

    private fun createConfigDriftScenario() = scenario("config") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "order-service", "order-pod-1", "error",
                "Connection refused to orders-db-old.prod.svc",
                mapOf("db.host" to "orders-db-old.prod.svc")),
            LogEntry("2", now, "order-service", "order-pod-2", "info",
                "Order created successfully",
                mapOf("db.host" to "orders-db-new.prod.svc"))
        ))
    }

    private fun createThirdPartyScenario() = scenario("3rdparty") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "checkout-service", "checkout-1", "error",
                "HTTP 503 from Stripe: Service Unavailable",
                mapOf("provider" to "stripe", "http.status_code" to 503))
        ))
    }

    private fun createRateLimitScenario() = scenario("ratelimit") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "api-gateway", "gw-1", "warn",
                "Rate limit bucket exhausted, rejecting requests",
                mapOf("rate_limit.rejection_count" to 500))
        ))
    }

    private fun createElasticsearchScenario() = scenario("es") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "elasticsearch", "es-1", "error",
                "Cluster health changed: GREEN -> RED, unassigned shards: 30",
                mapOf("cluster.health" to "RED", "unassigned_shards" to 30))
        ))
    }

    private fun createInvestigationLatencyScenario() = scenario("inv-latency") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "api-service", "api-1", "error",
            ...
            LogEntry("2", now, "inventory-service", "inv-1", "error",
                "Query timeout: SELECT * FROM inventory",
                mapOf("error.type" to "QueryTimeoutException")),
            LogEntry("3", now.minus(45, ChronoUnit.MINUTES), "postgresql", "db-1", "warn",
                "Autovacuum running on table 'inventory' - blocking queries",
                mapOf("db.operation" to "autovacuum"))
        ))
    }

    private fun createInvestigationErrorsScenario() = scenario("inv-errors") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "order-service", "order-1", "error",
                "Authentication failed: 401 Unauthorized",
                mapOf("http.status_code" to 401)),
            LogEntry("2", now, "auth-service", "auth-1", "warn",
                "Token validation failed: issuer mismatch. Expected: https://auth-v2.example.com",
                mapOf("validation.error" to "issuer_mismatch"))
        ))
        events(listOf(
            EventEntry(1, "Config Update: auth-service-config",
                "Removed old issuer from allowed_issuers",
                now.minus(5, ChronoUnit.MINUTES), source = "kubectl",
                tags = listOf("config", "auth-service"))
        ))
    }

    private fun createInvestigationIntermittentScenario() = scenario("inv-intermittent") {
        val now = Instant.now()
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        incidentLogs(listOf(
            LogEntry("1", now, "checkout-service", "checkout-ghi44", "error",
                "DNS lookup failed: payment-service.production.svc.cluster.local: Name does not resolve",
                mapOf("kubernetes.pod.name" to "checkout-ghi44", "kubernetes.node.name" to "worker-node-3")),
            LogEntry("2", now, "node-local-dns", "worker-node-3", "error",
                "NodeLocal DNSCache crashed: OOM",
                mapOf("kubernetes.event.reason" to "OOMKilled", "kubernetes.node.name" to "worker-node-3"))
        ))
    }
}
