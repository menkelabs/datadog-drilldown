...
class DiceRcaIntegrationTest {
    ...
    @Test
    @Disabled("Requires DICE server running")
    fun `should identify database pool exhaustion from latency alert`() {
        ...
        println("Loaded ${loadResult.documentsLoaded} documents with ${loadResult.propositionsExtracted} propositions")
        
        // Step 2: Setup mock Datadog data
        mockDatadog.loadScenario("db-pool-exhaustion", createDbPoolExhaustionScenario())
        ...
        val alert = TestAlert(
            ...
            metricQuery = "p95:trace.api-service.request.duration{env:prod}",
            currentValue = 2450.0,
            threshold = 500.0,
        ...
        val result = testFramework.simulateAlert(contextId, alert, "db-pool-exhaustion")
        
        // Step 4: Verify conclusion
        val verification = testFramework.verifyConclusion(
            result,
            ExpectedRootCause(
                keywords = listOf("connection pool", "database", "exhausted", "HikariPool", "timeout"),
                component = "database",
    ...
    fun `should identify downstream failure from error rate alert`() {
        ...
        // Step 2: Setup mock Datadog data
        mockDatadog.loadScenario("downstream-failure", createDownstreamFailureScenario())
        ...
        val alert = TestAlert(
            ...
            metricQuery = "sum:trace.order-service.request.errors{env:prod}.as_rate()",
            currentValue = 23.0,
            threshold = 5.0,
            metricsToQuery = listOf(
        ...
        val result = testFramework.simulateAlert(contextId, alert, "downstream-failure")
        
        val verification = testFramework.verifyConclusion(
            result,
            ExpectedRootCause(
                keywords = listOf("payment-service", "downstream", "circuit breaker", "unavailable"),
        ...
        println(verification.summary())
        assertTrue(verification.passed, "Should identify downstream service failure")
    }
    ...
    fun `should identify config change from auth error alert`() {
        ...
        // Step 2: Setup mock Datadog data
        mockDatadog.loadScenario("auth-config-change", createAuthConfigChangeScenario())
        ...
        val alert = TestAlert(
            ...
            traceQueries = listOf("order-service" to "prod", "auth-service" to "prod")
        )
        
        val result = testFramework.simulateAlert(contextId, alert, "auth-config-change")
        
        val verification = testFramework.verifyConclusion(
            result,
            ExpectedRootCause(
                keywords = listOf("config", "issuer", "OAuth", "auth-service", "allowed"),
        ...
        println(verification.summary())
        assertTrue(verification.passed, "Should identify config change as root cause")
    }
    ...
    private fun createDatabaseScenarioPriorKnowledge() = PriorKnowledge(
        architecture = SystemArchitecture(
            ...
            services = listOf(
                ServiceInfo(
                    ...
                    type = "gateway",
                    dependencies = listOf("inventory-service", "order-service", "postgres"),
                    critical = true
                ),
                ServiceInfo(
                    name = "postgres",
        ...
        dependencies = listOf(
            ServiceDependency(
                ...
                type = "database",
                critical = true,
                notes = "Uses HikariCP connection pool, max 100 connections"
            )
        ),
        failurePatterns = listOf(
            FailurePattern(
                ...
                description = "Connection pool becomes saturated when queries are slow or connections aren't released",
                symptoms = listOf(
                ...
                rootCause = "Database connection pool exhausted due to slow queries or connection leaks",
                resolution = "1. Identify slow queries. 2. Check for connection leaks. 3. Increase pool size if needed.",
                affectedServices = listOf("api-service", "order-service")
            )
        ),
        pastIncidents = listOf(
            PastIncident(
                id = "INC-2024-001",
                title = "API Latency Spike Due to DB Pool Exhaustion",
                date = "2024-12-15",
                durationMinutes = 45,
                severity = "HIGH",
                symptoms = listOf(
                ...
                rootCause = "A new query was holding connections too long after a deployment",
                resolution = "Rolled back deployment and optimized the query",
    ...
    private fun createDownstreamScenarioPriorKnowledge() = PriorKnowledge(
        architecture = SystemArchitecture(
            name = "E-Commerce Platform",
            description = "Microservices-based e-commerce system",
            services = listOf(
                ServiceInfo("order-service", "Handles order processing", "service",
                    listOf("payment-service", "inventory-service"), true),
                ServiceInfo("payment-service", "Processes payments via Stripe", "service",
                    listOf("stripe-api", "payment-db"), true)
            )
        ),
        dependencies = listOf(
            ServiceDependency("order-service", "payment-service", "http", true,
        ...
        failurePatterns = listOf(
            FailurePattern(
                ...
                description = "When a downstream service fails, it can cascade to upstream services",
                symptoms = listOf(
                ...
                rootCause = "Downstream service failure causing cascade",
                resolution = "1. Identify failing downstream service. 2. Check circuit breaker status. 3. Investigate root cause in downstream.",
                affectedServices = listOf("order-service", "checkout-service")
            )
        ),
        pastIncidents = listOf(
            PastIncident(
                id = "INC-2024-002",
                title = "Order Failures Due to Payment Service Outage",
                date = "2024-11-20",
                durationMinutes = 30,
                severity = "CRITICAL",
                symptoms = listOf(
                ...
                rootCause = "Payment service database was unreachable",
                resolution = "Fixed network connectivity to payment-db",
                lessonsLearned = listOf(
    ...
    private fun createAuthScenarioPriorKnowledge() = PriorKnowledge(
        architecture = SystemArchitecture(
            name = "E-Commerce Platform",
            description = "Microservices with centralized auth",
            services = listOf(
                ServiceInfo("order-service", "Order management", "service",
                    listOf("auth-service"), true),
                ServiceInfo("auth-service", "OAuth2 authentication", "auth",
                    listOf("identity-db"), true)
            )
        ),
        dependencies = listOf(
        ...
        failurePatterns = listOf(
            FailurePattern(
                ...
                description = "Auth configuration changes can invalidate existing tokens",
                symptoms = listOf(
                ...
                rootCause = "Configuration change in auth-service (e.g., allowed issuers)",
                resolution = "Review recent config changes to auth-service",
                affectedServices = listOf("All services using auth-service")
            )
        ),
        pastIncidents = listOf(
            PastIncident(
                ...
                rootCause = "ConfigMap update removed old OAuth issuer from allowed list",
                resolution = "Added old issuer back, implemented gradual migration",
                lessonsLearned = listOf(
    ...
    private fun createDbPoolExhaustionScenario() = scenario("db-pool-exhaustion") {
        val now = Instant.now()
        ...
        monitor(MonitorResponse(
            ...
            type = "metric alert",
            query = "p95:trace.api-service.request.duration{env:prod} > 500",
            tags = listOf("service:api-service", "env:prod")
        ))
        
        baselineMetrics(MetricResponse(series = listOf(
            MetricSeries(metric = "trace.api-service.request.duration",
                pointlist = listOf(MetricPoint(now.minus(60, ChronoUnit.MINUTES).toEpochMilli(), 50.0)))
        )))
        
        incidentMetrics(MetricResponse(series = listOf(
            MetricSeries(metric = "trace.api-service.request.duration",
                pointlist = listOf(MetricPoint(now.toEpochMilli(), 2450.0)))
        )))
        
        incidentLogs(listOf(
            LogEntry(id = "log-1", timestamp = now, service = "api-service", status = "error",
                message = "TimeoutError: Connection pool exhausted after 5000ms",
                attributes = mapOf("error.type" to "TimeoutError")),
            LogEntry(id = "log-2", timestamp = now, service = "api-service", status = "error",
                message = "HikariPool-1: Connection not available, request timed out after 5000ms",
                attributes = mapOf("pool.name" to "HikariPool-1"))
        ))
    }

    private fun createDownstreamFailureScenario() = scenario("downstream-failure") {
        val now = Instant.now()
        description("Downstream payment service failure")
        incidentStart(now.minus(30, ChronoUnit.MINUTES))
        
        monitor(MonitorResponse(
            id = 23456, name = "Order Service Error Rate", type = "metric alert",
            query = "sum:trace.order-service.request.errors{env:prod}.as_rate() > 5",
            tags = listOf("service:order-service")
        ))
        
        incidentLogs(listOf(
            LogEntry(id = "log-1", timestamp = now, service = "order-service", status = "error",
                message = "Failed to connect to payment-service: Connection timeout",
                attributes = mapOf("downstream.service" to "payment-service")),
            LogEntry(id = "log-2", timestamp = now, service = "order-service", status = "warn",
                message = "CircuitBreaker 'payment-service' state changed: CLOSED -> OPEN",
                attributes = mapOf("circuit.name" to "payment-service"))
        ))
    }

    private fun createAuthConfigChangeScenario() = scenario("auth-config-change") {
        val now = Instant.now()
        ...
        incidentLogs(listOf(
            ...
            LogEntry(id = "log-2", timestamp = now, service = "auth-service", status = "warn",
                message = "Token validation failed: issuer mismatch. Token issuer: https://auth.example.com, Expected: https://auth-v2.example.com",
                attributes = mapOf("validation.error" to "issuer_mismatch"))
        ))
        
        events(listOf(
            EventEntry(id = 1, title = "Config Update: auth-service-config",
                text = "Updated allowed_issuers - removed https://auth.example.com",
                dateHappened = now.minus(5, ChronoUnit.MINUTES),
                source = "kubectl", tags = listOf("config", "auth-service"))
        ))
    }
    ...
    private fun createTestDiceClient(): DiceClient {
        ...
        return DiceClient(diceServerUrl = System.getenv("DICE_SERVER_URL") ?: "http://localhost:8080")
    }
}
